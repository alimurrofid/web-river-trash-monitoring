...
export const createStreamingLink = async (req: Request, res: Response) => {
  try {
    ...
    // Generate link tanpa waktu kedaluwarsa
    ...
    const streamingId = await streamingRepository.createStreamingLink(
      streamingData
    );
...
export const getActiveStreamingLinkByBillboard = async (
  ...
  try {
    ...
    const streamingLink =
      await streamingRepository.getActiveStreamingLinkByBillboard(billboard);
...
export const updateStreamingLink = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { link, billboard_name } = req.body;
    ...
    if (link && !link.startsWith("http://") && !link.startsWith("https://")) {
      return res.status(400).json({
        success: false,
        message:
          "Format link tidak valid (harus dimulai dengan http:// atau https://)",
      });
    }
    ...
    const updatedLink = await streamingRepository.updateStreamingLink(
      parseInt(id),
      updateData
    );
...
export const validateStreamingLink = async (req: Request, res: Response) => {
  try {
    ...
    const streamingLink =
      await streamingRepository.getActiveStreamingLinkByBillboard(
        billboardName
      );
